...
namespace SmartBot.Mulligan
{
    ...
    */
    ...
    public class Universal : MulliganProfile
    {
        ...
        private const string SBuild = "0025";                               // current build version
            ...
            Card.CClass ownClass)
        {
            ...
            DeckInfo currentDeckInfo = new DeckInfo(_myDeck);

            //ARCHETYPE DETECTOR
            
            var archetypes = new List<Archetype>();
            archetypes.AddRange(Bot.GetArchetypes().Where(archetype => archetype.ArchetypeName().Contains("SMK_")));
            Archetype friend = ArchetypeDetector.DetectArchetype(_myDeck, archetypes, 70);
            if (BlnDebugMode)
            {
                string s = "";
            ...
            IniManager currentFile = _iniTierList;
            ...
            { 
                ...
                {
                    string tempArgs = currentFile.GetString("combo", string.Format("combo_{0}", i), null);
            ...
            int[] iCardCombo = new int[_choices.Count];
            ...
            for (var i = 0; i < choices.Count; i++)
            {
                ...
                int.TryParse(
                    _iniTierList.GetString(string.Format("card_{0}", card), string.Format("class_{0}", ownClass), "-1917"), 
                    out temp);

                iCardPts[i] = (temp == -1917) ? iCardPts[i] : temp;

                int.TryParse(
                    _iniTierList.GetString(string.Format("card_{0}", card), "combo", "0"),
                    out iCardCombo[i]);

                int.TryParse(
                    _iniTierList.GetString(string.Format("card_{0}", card),  string.Format("opp_{0}", _opponentClass), "0"), 
                    out temp);
            ...
            do
            {
                ...
                for (i[3] = 0; (i[3] < 2 && coin || i[3] < 1 && !coin); i[3]++)
                {
                    ...
                    if (!combinationCmb[i[0], i[1], i[2], i[3]])
                    {
                        ...
                        {
                            combinationPts[i[0], i[1], i[2], i[3]] -= (combinationCst[i[0], i[1], i[2], i[3], 2] - 1) * 20;
                        }
                    }
