...
if ((board.EnemyAbility.Template.Id == Card.Cards.AT_132_PALADIN
        ...
        || (board.MinionEnemy.Any(minion => minion.Template.Id == Card.Cards.TRL_520))//如果对面场上有鱼人大厨
        ...
        || (board.MinionEnemy.Any(minion => minion.Template.Id == Card.Cards.BT_197t))//如果对面场上有终极魂匣
                ...
                && myAttack <= 5)//场攻小于等于5
            {
                ...
                if (
                    board.ManaAvailable == 6   //有效水晶等于6
                    && board.MinionFriend.Any(card => card.Race == Card.CRace.TOTEM)   //如果场上有图腾种族随从
                    && board.MinionFriend.Count >= 3  //场上随从数量大于等于3
                    && myAttack <= 6 //场攻等于或者小于6
                    && board.Hand.Exists(x => x.Template.Id == Cards.TheStormBringer)//手上有聚合器
                   )
                {
                    p.CastSpellsModifiers.AddOrUpdate(Cards.TheStormBringer, new Modifier(-500));//提高风暴聚合器优先级
                }

                if (
                    board.ManaAvailable == 7    //有效水晶等于7
                    && board.MinionFriend.Any(card => card.Race == Card.CRace.TOTEM)   //如果场上有图腾种族随从
                    && board.MinionFriend.Count >= 3  //场上随从数量大于等于3
                    && myAttack <= 6 //场攻等于或者小于6
                    && board.Hand.Exists(x => x.Template.Id == Cards.TheStormBringer)//手上有聚合器
                ...
                if (
                    board.ManaAvailable == 8   //有效水晶等于8
                    && board.MinionFriend.Any(card => card.Race == Card.CRace.TOTEM)   //如果场上有图腾种族随从
                    && board.MinionFriend.Count >= 2  //场上随从数量大于或等于2
                    && myAttack <= 6 //场攻等于或者小于6
                    && board.Hand.Exists(x => x.Template.Id == Cards.TheStormBringer)//手上有聚合器
                   )
                {
                ...
                if (
                    board.ManaAvailable == 10   //有效水晶等于10
                    && board.MinionFriend.Any(card => card.Race == Card.CRace.TOTEM)   //如果场上有图腾种族随从
                    && board.MinionFriend.Count >= 1  //场上随从数量大于或等于1
                    && myAttack <= 6 //场攻等于或者小于6
                    && board.Hand.Exists(x => x.Template.Id == Cards.TheStormBringer)//手上有聚合器
                   )
                {
                ...
                //我方
                //降低鱼斯拉优先级
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BT_230, new Modifier(200));
                //降低龙族跟班优先级
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DRG_052, new Modifier(60));

                //法术
                //连环爆裂
                p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GVG_038, new Modifier(40));
