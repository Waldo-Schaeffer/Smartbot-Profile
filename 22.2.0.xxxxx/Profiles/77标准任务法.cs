      ...
      public ProfileParameters GetParameters(Board board)
      {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            // 火焰法术使用数量
            int huoyanfashu=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SCH_348)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SW_107)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.CS2_029)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SW_110)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SW_108);
            Bot.Log("火焰法术使用数量"+huoyanfashu);
            // 奥术法术使用数量
            int aoshufashu=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BT_002)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.CS2_023)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_541)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SCH_353)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SCH_270);
            Bot.Log("奥术法术使用数量"+aoshufashu);
            // 冰霜法术
            int bingshaungfashu=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.CORE_EX1_289)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.SCH_509)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_305t2)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_305t)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_305);
            Bot.Log("冰霜法术使用数量"+bingshaungfashu);
...
#region Card.Cards.HERO_08bp 英雄技能
        p.PlayOrderModifiers.AddOrUpdate(Card.Cards.HERO_08bp, new Modifier(-9999)); 
        p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.HERO_08bp, new Modifier(130));
#endregion

#region 咒术洪流 BT_002
        if( board.HasCardInHand(Card.Cards.BT_002))
        {          
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_002,new Modifier(-99));
          Bot.Log("咒术洪流 -99");
        } 
#endregion
#region 考前刷夜 SCH_353
        if( board.HasCardInHand(Card.Cards.SCH_353))
        {          
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_353,new Modifier(-99));
          Bot.Log("考前刷夜 -99");
        } 
#endregion

#region 清凉的泉水 BAR_542
        if( board.HasCardInHand(Card.Cards.BAR_542))
        {          
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_542,new Modifier(-9999));
          Bot.Log("清凉的泉水 -9999");
        } 
#endregion

#region 埃匹希斯冲击 BT_291
        if( board.HasCardInHand(Card.Cards.BT_291))
        {          
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_291,new Modifier(130));
          Bot.Log("埃匹希斯冲击 130");
        } 
#endregion

#region 套圈圈 DMF_105
        if( board.HasCardInHand(Card.Cards.DMF_105))
        {          
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_105,new Modifier(130));
          Bot.Log("套圈圈 130");
        } 
#endregion
#region 套圈圈 DMF_105t
        if( board.HasCardInHand(Card.Cards.DMF_105t))
        {          
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_105t,new Modifier(-99));
          Bot.Log("套圈圈 -99");
        } 
#endregion
#region 奥术师晨拥 SW_450t4 
       p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_450t4, new Modifier(-9999));
       p.PlayOrderModifiers.AddOrUpdate(Card.Cards.SW_450t4, new Modifier(9999));
#endregion

#region 冰霜
if((bingshaungfashu<aoshufashu
||bingshaungfashu<huoyanfashu)
&&bingshaungfashu==0
){
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_EX1_289, new Modifier(-99));//寒冰护体 CORE_EX1_289
Bot.Log("冰霜 -99");
}
if((bingshaungfashu<aoshufashu
||bingshaungfashu<huoyanfashu)
...
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_509, new Modifier(-99));//冰冷智慧 SCH_509
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_305, new Modifier(-99));//冰风暴（等级1）BAR_305
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_305t, new Modifier(-99));//冰风暴（等级1）BAR_305t
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_305t2, new Modifier(-99));//冰风暴（等级1）BAR_305t2
Bot.Log("冰霜 -99");
}
#endregion

#region 奥术
if(aoshufashu<bingshaungfashu
||aoshufashu<huoyanfashu
||aoshufashu==0
){
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_270, new Modifier(-99));//始生研习 SCH_270
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_353, new Modifier(-99));//考前刷夜 SCH_353
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_541, new Modifier(-99));//符文宝珠 BAR_541
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CS2_023, new Modifier(-99));//奥术智慧 CS2_023
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_002, new Modifier(-99));//咒术洪流 BT_002
Bot.Log("奥术 -99");
}
#endregion

#region 火焰
if(huoyanfashu<bingshaungfashu
||huoyanfashu<aoshufashu
||huoyanfashu==0
){
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_110, new Modifier(-99));//点燃 SW_110 
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_CS2_029, new Modifier(-99));//火球术 CORE_CS2_029
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_107, new Modifier(-99));//火热促销 SW_107
Bot.Log("火焰 -99");
}
if((huoyanfashu<bingshaungfashu
||huoyanfashu<aoshufashu)
...
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_108, new Modifier(-99));//初始之火 SW_108
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_348, new Modifier(-99));//燃烧 SCH_348
Bot.Log("火焰 -99");
}
#endregion
