...
// 提高日蚀优先级
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_058, new Modifier(-10));//日蚀 Solar Eclipse ID：DMF_058
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BAR_539, new Modifier(-9999));//超凡之盟 Celestial Alignment ID：BAR_539
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_128, new Modifier(-99));//真菌宝藏
...
if(board.HasCardOnBoard(Card.Cards.DMF_734)&&board.MinionFriend.Count >1){
...
// 萌芽分裂 Germination ID：BT_129优先给安娜科德拉 Lady Anacondra ID：WC_006
p.CastSpellsModifiers.AddOrUpdate(Card.Cards.BT_129, new Modifier(-40, Card.Cards.WC_006));
// // 如果手上没有萌芽分裂 不下安娜
// if(board.HasCardInHand(Card.Cards.BT_129)&&board.HasCardInHand(Card.Cards.WC_006)){
// p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WC_006, new Modifier(-250));
// }
// 随从数量大于等于三提高丛林之魂 Soul of the Forest ID：CORE_EX1_158
if(board.MinionFriend.Count >3&&board.HasCardInHand(Card.Cards.CORE_EX1_158)){
  p.CastSpellsModifiers.AddOrUpdate(Card.Cards.CORE_EX1_158, new Modifier(-20));
}
...
// 手上有法师长袍,场上随从大于等于2,提高长袍优先级
      if(board.MinionFriend.Count >=1&&board.HasCardInHand(Card.Cards.SCH_146)){
...
if(
 ( board.HasCardInHand(Card.Cards.SW_437)//施肥 Composting ID：SW_437
||board.HasCardInHand(Card.Cards.YOP_026)//树木生长 Arbor Up ID：YOP_026
)&&
board.HasCardInHand(Card.Cards.SW_060)//卖花女郎 Florist ID：SW_060 
){
  p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_060, new Modifier(-20));//卖花女郎 Florist ID：SW_060
}
