...
#region 攻击模式和自定义 
      public ProfileParameters GetParameters(Board board)
      {
            ...
            else
            {
               ...
               p.GlobalAggroModifier = (int)(a * 0.625 + 80.5);
            }

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            // 海盗数量
            int haidaonum=board.Hand.Count(card => card.Type == Card.CType.MINION)-board.Hand.Count(x => x.Template.Id == Card.Cards.BAR_075)-board.Hand.Count(x => x.Template.Id == Card.Cards.SCH_713)-board.Hand.Count(x => x.Template.Id == Card.Cards.GVG_075)-board.Hand.Count(x => x.Template.Id == Card.Cards.DRG_023)-board.Hand.Count(x => x.Template.Id == Card.Cards.FP1_030)-board.Hand.Count(x => x.Template.Id == Card.Cards.SCH_142)-board.Hand.Count(x => x.Template.Id == Card.Cards.UNG_807);
            Bot.Log("海盗数量"+haidaonum);
            // 除去空降匪徒
            int filterHaidao=haidaonum-board.Hand.Count(x => x.Template.Id == Card.Cards.DRG_056);
            Bot.Log("除去空降匪徒海盗数量"+haidaonum);
            int changshanghaidaonum=board.MinionFriend.Count(card => card.Type == Card.CType.MINION)-board.MinionFriend.Count(x => x.Template.Id == Card.Cards.BAR_075)-board.MinionFriend.Count(x => x.Template.Id == Card.Cards.DRG_023)-board.MinionFriend.Count(x => x.Template.Id == Card.Cards.GVG_075);
            ...
            int baonusuicong=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_896)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_025)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_843);
            Bot.Log("坟场暴怒随从数量"+baonusuicong);
 #endregion
...
#region 洛卡拉 Rokara ID：BAR_847 
            if(board.HasCardInHand(Card.Cards.BAR_847)
            &&board.MinionFriend.Count ==0
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_847, new Modifier(150)); 
            Bot.Log("洛卡拉 150");
            }
#endregion
#region 防护长袍 SCH_146
...
#region 前沿哨所 BAR_074
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_074, new Modifier(-40)); 
            Bot.Log("前沿哨所 -40");
            }
#endregion
...
#region 货物保镖 SW_030
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_030, new Modifier(-99)); 
            Bot.Log("货物保镖 -99");
            }
#endregion
#region 血帆桨手 CS3_008 
...
#endregion
#region 氮素药膏 Nitroboost Poison  YOP_015 
            if(board.HasCardInHand(Card.Cards.YOP_015)
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.YOP_015, new Modifier(150));
            Bot.Log("氮素药膏 150");
            }
#endregion
#region 氮素药膏 YOP_015t 
            if(board.HasCardInHand(Card.Cards.YOP_015t)
            &&board.HasCardInHand(Card.Cards.SCH_623)//劈砍课程 SCH_623 
            &&board.WeaponFriend != null
            ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.YOP_015, new Modifier(-40));
            Bot.Log("腐蚀过的氮素药膏 -40");
            }
#endregion
...
#endregion
#region 海盗之锚 DRG_025
          // if(board.HasCardInHand(Card.Cards.DRG_025)
          //   &&board.HasCardInHand(Card.Cards.OG_312)//恩佐斯的副官 OG_312
          //   ){
          //   p.CastMinionsModifiers.AddOrUpdate(Card.Cards.OG_312, new Modifier(999)); 
          //   Bot.Log("恩佐斯的副官 999 ");
          //   }
          ...
          //   ){
          //   p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_521, new Modifier(150)); 
          //   Bot.Log("吞剑艺人 150 ");
          //   }
          if(board.HasCardInHand(Card.Cards.DRG_025)
            
          ){
            p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.DRG_025, new Modifier(-20));
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.SW_028t5, new Modifier(999)); 
            Bot.Log("海盗之锚 -20 优先值 480");
          }  
          if(board.WeaponFriend != null 
          && board.WeaponFriend.Template.Id == Card.Cards.DRG_025
          &&board.HasCardInHand(Card.Cards.EX1_409)//升级 EX1_409
          ){
           p.PlayOrderModifiers.AddOrUpdate(Card.Cards.EX1_409, new Modifier(9999)); 
            p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.DRG_025, new Modifier(250));
            Bot.Log("升级 -9999");
          }  
          ...
          // &&luokala==0
          // ){
          //   p.GlobalWeaponsAttackModifier = 9999;
          //    p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.DRG_025, new Modifier(9999));
          //   Bot.Log("攻击优先级 9999");
          // }  
          // if(board.WeaponFriend != null 
          // && board.WeaponFriend.Template.Id == Card.Cards.DRG_025
          // &&board.HasCardInHand(Card.Cards.OG_315)
          // &&luokala==0
          // &&changshanghaidaonum>= 1
          // ){
          //   p.GlobalWeaponsAttackModifier = 9999;
          //   p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.DRG_025, new Modifier(9999));
          //   Bot.Log("攻击优先级 9999");
          ...
          if(board.WeaponFriend != null 
          && board.WeaponFriend.Template.Id == Card.Cards.DRG_025
          &&board.HasCardInHand(Card.Cards.OG_315)//血帆教徒 Bloodsail Cultist      OG_315 
          &&changshanghaidaonum>= 1
          ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.OG_315, new Modifier(-9999)); 
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.OG_315, new Modifier(9999)); 
            // p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.DRG_025, new Modifier(9999));
            Bot.Log("血帆教徒 -9999");
          }  
...
#region 吞剑艺人 Sword Eater      DMF_521 
        if(board.HasCardInHand(Card.Cards.DMF_521)
            ...
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.DMF_521, new Modifier(-20)); 
  
            Bot.Log("吞剑艺人 -20 ");
            }
#endregion
#region 太阳之井新兵 BAR_025
        if(board.HasCardInHand(Card.Cards.BAR_025)
         ){
            p.PlayOrderModifiers.AddOrUpdate(Card.Cards.BAR_025, new Modifier(-20)); 
  
            Bot.Log("太阳之井新兵 -20 ");
            }
#endregion
#region 厚重板甲  SW_094 
        if(board.HasCardInHand(Card.Cards.SW_094)
        &&board.HeroFriend.CurrentHealth<=10
         ){
         p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_094, new Modifier(-99));
            Bot.Log("厚重板甲 -99 ");
            }
#endregion
#region 萨鲁法尔大王 BAR_334   
        if(board.HasCardInHand(Card.Cards.BAR_334)
        &&baonusuicong<=1
        ){
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_334, new Modifier(999));
            Bot.Log("萨鲁法尔大王 999");
            }
#endregion
#region 雷区挑战  DMF_522  
        // if(board.HasCardInHand(Card.Cards.DMF_522)
        
        //  ){
        //   p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_522, new Modifier(130));
        //   Bot.Log("雷区挑战 130");
        //     }
#endregion


#region 锈蚀铁钩 Rusty Hook OG_058
        if(board.WeaponFriend != null 
          && board.WeaponFriend.Template.Id == Card.Cards.OG_058
         ){
            p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.OG_058, new Modifier(55));
            Bot.Log("锈蚀铁钩攻击值 -55");
          }  
        if(board.WeaponFriend != null 
          && board.WeaponFriend.Template.Id == Card.Cards.OG_058
          &&board.HasCardInHand(Card.Cards.OG_315)//OG_315
          &&board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.DRG_025)==0//海盗之锚 DRG_025 
         ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.OG_315, new Modifier(999)); 
            Bot.Log("血帆教徒 999");
          }  
#endregion

#region 断颚之刃 Jawbreaker ID：DMF_521t
        // if(board.WeaponFriend != null 
...
#region 收割之镰 SCH_238
        if(board.WeaponFriend != null 
          && board.WeaponFriend.Template.Id == Card.Cards.SCH_238
         ){
            p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_623, new Modifier(-99));//劈砍课程 SCH_623
            p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.SCH_238, new Modifier(130));
            Bot.Log("劈砍课程 -99");
          }  
#endregion
			
#region 恩佐斯的副官 OG_312
// if(board.HasCardInHand(Card.Cards.OG_312)
// &&board.WeaponFriend != null 
// && board.WeaponFriend.Template.Id == Card.Cards.OG_058//锈蚀铁钩 Rusty Hook OG_058
// &&board.ManaAvailable >= 4 
// &&!(
//   board.HasCardInHand(Card.Cards.SW_028t5)||board.HasCardOnBoard(Card.Cards.SW_028t5)
//       ||board.FriendGraveyard.Contains(Card.Cards.SW_028t5) 
// )
// ){
//             p.CastMinionsModifiers.AddOrUpdate(Card.Cards.OG_312, new Modifier(-99)); 
//             Bot.Log("恩佐斯的副官 -99 ");
//             }
if(board.HasCardInHand(Card.Cards.OG_312)
&&board.MaxMana <=3
...
#region 港口匪徒 SW_029 
         if(board.HasCardInHand(Card.Cards.SW_029)){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_029, new Modifier(-40)); 
           Bot.Log("港口匪徒 -40 ");
            }
#endregion
#region 问题学生 SCH_337
         if(board.HasCardInHand(Card.Cards.SCH_337)){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_337, new Modifier(-99)); 
           Bot.Log("问题学生 -99 ");
            }
#endregion
#region 暴风城海盗 Stormwind Freebooter SW_093 
        //  if(board.HasCardInHand(Card.Cards.SW_093)){
        //     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_093, new Modifier(130)); 
        //    Bot.Log("暴风城海盗 130 ");
        //     }
#endregion
...
#region 船长洛卡拉 SW_028t5 
          ...
          if(
            ...
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CS3_008, new Modifier(-999)); 
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.CFM_325, new Modifier(-999)); 
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.NEW1_025, new Modifier(-999)); 
            Bot.Log("提高做任务的优先级 ");
          }
#endregion
