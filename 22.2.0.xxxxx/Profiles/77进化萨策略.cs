...
#region 雷霆绽放 SCH_427  硬币 GAME_005
      ...
      if(board.MaxMana==5
       ...
       (
          (
            ...
            &&board.HasCardInHand(Card.Cards.SW_034)//小巧玩具 SW_034
            &&board.HasCardInHand(Card.Cards.DMF_700)//异变轮转 DMF_700
          )
      ...
      )
      {
        int friendCount=board.MinionFriend.Count;
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_427, new Modifier(-40));//雷霆绽放 SCH_427
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_034, new Modifier(-550));//小巧玩具 SW_034
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_700, new Modifier(-60));//异变轮转 DMF_700
          Bot.Log("5费用雷霆绽放和小巧玩具,异变转轮的combo");
      }
      ...
      {
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_427, new Modifier(250));///雷霆绽放 SCH_427
          Bot.Log("小于等于5费不用雷霆绽放");
      }
      ...
      {
        p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(250));///硬币 SCH_427
          Bot.Log("小于等于4费不用硬币");
      }
      ...
      if(board.MaxMana==4
       ...
       {
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_427, new Modifier(-40));//雷霆绽放 SCH_427
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(-40));//硬币 GAME_005
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_034, new Modifier(-999));//小巧玩具 SW_034
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_700, new Modifier(-200));//异变轮转 DMF_700
                  p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.BT_102,new Modifier(500));
          Bot.Log("4费用雷霆绽放,硬币和小巧玩具,异变转轮的combo");
       }

        if(board.MaxMana==4
          &&board.HasCardInHand(Card.Cards.GAME_005)//硬币 GAME_005
          &&board.HasCardInHand(Card.Cards.BT_102)//沼泽拳刺 BT_102
        )
        {
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(-10));//硬币 GAME_005
          p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.BT_102, new Modifier(-999));
          Bot.Log("4费用硬币和沼泽拳刺的combo");
        }
        ...
        {
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_427, new Modifier(15));///雷霆绽放 SCH_427
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(15));//硬币 GAME_005
          Bot.Log("大于等于6可以用硬币和雷霆绽放");
        }
        ...
        if(board.MaxMana==5
         ...
         )
         {
          ...
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_034, new Modifier(-999));//小巧玩具 SW_034
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_700, new Modifier(-200));//异变轮转 DMF_700
          Bot.Log("五费用雷霆绽放+小巧玩具+异变转轮的combo");
         }
        ...
        if(board.MaxMana==5
         ...
         )
         {
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(-40));//硬币 GAME_005
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_034, new Modifier(-999));//小巧玩具 SW_034
          Bot.Log("五费有刀用硬币+小巧玩具的combo");
         }

        if(board.HasCardOnBoard(Card.Cards.SW_062)// 闪金镇豺狼人 SW_062
         &&(board.HasCardInHand(Card.Cards.SCH_427)//雷霆绽放 SCH_427
         ||board.HasCardInHand(Card.Cards.GAME_005)//硬币 GAME_005
         )
         &&board.HasCardInHand(Card.Cards.DMF_700)//异变轮转 DMF_700
...
#region 前沿哨所 Far Watch Post ID：BAR_074
      if(
...
#region 笼斗管理员 DMF_704
      if(board.HasCardInHand(Card.Cards.DMF_704)
      )
      {
         p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_704, new Modifier(-60));
         Bot.Log("笼斗管理员-60");
      }
#endregion

#region 集合石 WC_028
      if(board.HasCardInHand(Card.Cards.WC_028)
        &&board.MaxMana ==1
        ){
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WC_028, new Modifier(-60));
            Bot.Log("集合石-60");
        }     
        if(board.HasCardInHand(Card.Cards.WC_028)
        &&board.MaxMana >=2
        ){
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WC_028, new Modifier(-10));
            Bot.Log("集合石-10");
        }     
        if(board.HasCardInHand(Card.Cards.WC_028)
        &&board.MaxMana >=2
        &&board.HasCardInHand(Card.Cards.SW_062)//闪金镇豺狼人 SW_062
        ){
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.WC_028, new Modifier(-60));
            Bot.Log("集合石-20");
        }     
...
#region 魔杖工匠 SCH_160
      if(board.HasCardInHand(Card.Cards.SCH_160)
      ){
         p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SCH_160, new Modifier(-20));
          Bot.Log("魔杖工匠-10");
      }
#endregion
...
#region 运河慢步者 SW_033
      ...
      if(board.MaxMana ==4
      &&board.HasCardInHand(Card.Cards.BT_102)//沼泽拳刺 BT_102
      &&board.HasCardInHand(Card.Cards.SW_033)
      ){
       p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_033, new Modifier(500)); 
          Bot.Log("手上有刀,4费不用运河慢步者");
      }

      //  if(board.HasCardInHand(Card.Cards.SW_033)&&board.MinionEnemy.Count == 0
      // ){
      //     p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_033, new Modifier(250)); 
      //     Bot.Log("降低运河慢步者优先级");
      // }
#endregion
...
#region 沼泽拳刺 BT_102
      ...
      &&board.MinionFriend.Count >= 2
        ...
        int friendnumber=board.MinionFriend.Count;
        p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.OG_058, new Modifier(85));
        Bot.Log("友方随从大于等于3可以砍,否则不砍");
      }
      if((board.WeaponFriend != null && board.WeaponFriend.Template.Id == Card.Cards.BT_102)
      &&(board.MinionFriend.Count <=1)
      ){
        p.WeaponsAttackModifiers.AddOrUpdate(Card.Cards.OG_058, new Modifier(9999));
        Bot.Log("不砍");
      }    
      // 手里有刀,提高小巧玩具 SW_034
      if(((board.WeaponFriend != null && board.WeaponFriend.Template.Id == Card.Cards.BT_102))
        &&board.HasCardInHand(Card.Cards.SW_034)
        &&board.MaxMana>=5
      ){
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SCH_427, new Modifier(-10));//雷霆绽放 SCH_427
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(-10));//硬币 GAME_005
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_034, new Modifier(-400));//小巧玩具 SW_034
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_033, new Modifier(150));//运河慢步者 SW_033 
          p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_062, new Modifier(150));//闪金镇豺狼人 SW_062
          Bot.Log("手里有刀,提高小巧玩具");
      }
      //手里有刀,降低 异变轮转 DMF_700
      if(((board.WeaponFriend != null && board.WeaponFriend.Template.Id == Card.Cards.BT_102))
        &&board.HasCardInHand(Card.Cards.DMF_700)
      ){
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_700, new Modifier(550));
          Bot.Log("手里有刀,降低 异变轮转");
      }
     
#endregion

#region 小巧玩具 SW_034
      if(board.HasCardInHand(Card.Cards.SW_034)
      &&board.MaxMana>=6
      ){
          p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_034, new Modifier(-200));//小巧玩具 SW_034
        p.CastWeaponsModifiers.AddOrUpdate(Card.Cards.BT_102, new Modifier(-500));

          Bot.Log("小巧玩具优先级提高如果美刀");
      }    
#endregion
...
#region 亚煞极，污染之源 DMF_188
      if(board.HasCardInHand(Card.Cards.DMF_188)//亚煞极，污染之源 DMF_188
      ||board.FriendGraveyard.Contains(Card.Cards.DMF_701t)//深水炸弹 Dunk Tank ID：DMF_701t 
      ){
         p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DMF_188, new Modifier(-40));
          Bot.Log("坟场有深水炸弹,有亚煞极,提高亚煞极优先级");
      }
#endregion

#region 闪金镇豺狼人 SW_062
          if(board.HasCardInHand(Card.Cards.SCH_244)){
            //如果手牌为0,费用为10,不考虑
            //如果手牌为1,费用为9,10费出如果有异变,如果有硬币9费出,如果有雷霆绽放8费出
            //如果手牌为x,费用为10-x,11-x费出如果有异变,如果有硬币10-x费出,如果有雷霆绽放9-x费出
            int countNumber=board.FriendDeckCount;
            int nowMana=10-countNumber;
            int leiting=9-countNumber;
              if(board.FriendDeckCount==countNumber//手牌数为x
                  &&board.HasCardInHand(Card.Cards.SCH_244)//有豺狼人
                ){
                  // 有异变的时候
                    if(board.HasCardInHand(Card.Cards.DMF_700))//手里有异变转轮
                    {
                          if(board.HasCardInHand(Card.Cards.GAME_005)//有硬币硬币 GAME_005
                          &&board.MaxMana==nowMana//有硬币10-x费出
                            ){
                              p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_062, new Modifier(-80));
                              p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_700, new Modifier(-40));
                              p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(-40));
                              Bot.Log("当手里有硬币时"+nowMana+"出豺狼人");
                            }
                          if(board.HasCardInHand(Card.Cards.SCH_427)//雷霆绽放 SCH_427
                          &&board.MaxMana==leiting//有雷霆绽放9-x费出
                            ){
                              p.CastMinionsModifiers.AddOrUpdate(Card.Cards.SW_062, new Modifier(-80));
                              p.CastSpellsModifiers.AddOrUpdate(Card.Cards.DMF_700, new Modifier(-40));
                              p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GAME_005, new Modifier(-40));
                              Bot.Log("当手里有雷霆绽放时"+leiting+"出豺狼人");
                            }
                    }else{
...
#region 永恒之火 Perpetual Flame WC_020 
       if(
        board.HasCardInHand(Card.Cards.WC_020)//硬币 GAME_005
        &&enemyAttack<=6
      )
      {
...
#region 锈烂蝰蛇 Rustrot Viper ID：SW_072
    if(
        board.HasCardInHand(Card.Cards.SW_072)//锈烂蝰蛇 Rustrot Viper ID：SW_072 
