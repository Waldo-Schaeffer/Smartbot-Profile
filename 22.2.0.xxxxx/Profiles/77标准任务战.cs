      ...
      public ProfileParameters GetParameters(Board board)
      {
...

       {
         ...
         //定义场攻  用法 myAttack <= 5 自己场攻大于小于5  enemyAttack  <= 5 对面场攻大于小于5  已计算武器伤害
            ...
            int baonusuicong=board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.WC_026)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_896)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_025)+board.FriendGraveyard.Count(card => CardTemplate.LoadFromId(card).Id == Card.Cards.BAR_843);
...
#region 摧心者 Mindbreaker ICC_902  
            if(board.HasCardInHand(Card.Cards.ICC_902)
            ){
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.ICC_902, new Modifier(-99)); 
            Bot.Log("摧心者 -99");
            }
#endregion
#region 货物保镖 SW_030
