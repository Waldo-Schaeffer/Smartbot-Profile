...
p.CastMinionsModifiers.AddOrUpdate(Card.Cards.BAR_074, new Modifier(-20));//前沿哨所 Far Watch Post ID：BAR_074 
...
// 不送钢鬃卫兵 Razormane Battleguard ID：BAR_537 
 p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.BAR_537, new Modifier(150)); 
// 送活泼的松鼠 Vibrant Squirrel ID：SW_439 
//  p.OnBoardFriendlyMinionsValuesModifiers.AddOrUpdate(Card.Cards.SW_439, new Modifier(-20)); 
//  当随从数量大于等于三时增加施肥 Composting ID：SW_437优先级
if((board.MinionFriend.Count >= 3&&board.HasCardInHand(Card.Cards.SW_437))||board.FriendDeckCount <= 3){
  p.CastSpellsModifiers.AddOrUpdate(Card.Cards.SW_437, new Modifier(-40));
}
// 增加废料场巨魔的攻击优先值
