...
namespace SmartBotProfiles
{
    ...
    public class WildDemonHunter : Profile
    {
        ...
        private static readonly Dictionary<Card.Cards, int> _spellDamagesTable = new Dictionary<Card.Cards, int>
        {
            ...
            {Card.Cards.GVG_038, 4},//连环爆裂 Crackle ID：GVG_038
        ...
        public ProfileParameters GetParameters(Board board)
        {
            ...
            //具体策略
            if (board.Hand.Exists(x => x.Template.Id == Cards.VilespineSlayer) && (board.EnemyClass != Card.CClass.PALADIN) || board.Hand.Exists(x => x.Template.Id == Cards.SI7Agent && board.EnemyClass != Card.CClass.PALADIN)) ;
            //如果你的手牌有军7刺杀花。.且敌人不是骑士
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.UNG_809t1, new Modifier(200));
            //火羽衍生物价值等于200

            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DAL_415, new Modifier(150));


            if (board.MaxMana == 2 && (board.EnemyClass != Card.CClass.PALADIN))
                //如果你的水晶=2
                p.CastHeroPowerModifier.AddOrUpdate(Card.Cards.HERO_03bp, new Modifier(-60));
            //武器优先级=-60



            if ((board.Hand.Exists(x => x.Template.Id == Card.Cards.DAL_747))
                && (board.EnemyClass == Card.CClass.PRIEST)
                )
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.DAL_747, new Modifier(-150));
            }
            //如果对面是牧师，提高飞行管理员的优先级


            if ((board.Hand.Exists(x => x.Template.Id == Card.Cards.EX1_085))
                && (board.MinionEnemy.Count <= 3)
                )
            {
                p.CastMinionsModifiers.AddOrUpdate(Card.Cards.EX1_085, new Modifier(300));
            }
            //130
            ...
            //军七可裸下
            p.CastMinionsModifiers.AddOrUpdate(Card.Cards.EX1_134, new Modifier(45));
          ...
          //  p.CastSpellsModifiers.AddOrUpdate(Card.Cards.GVG_038, new Modifier(134));//连环爆裂 Crackle  ID：GVG_038
          //  p.CastSpellsModifiers.AddOrUpdate(Card.Cards.LOOT_541t, new Modifier(-200));//国王的赎金 King's Ransom ID：LOOT_541t
            ...
            p.OnBoardBoardEnemyMinionsModifiers.AddOrUpdate(Card.Cards.KAR_092, new Modifier(50));
                ...
                p.OnBoardBoardEnemyMinionsModifiers.AddOrUpdate(Card.Cards.KAR_089, new Modifier(50));
